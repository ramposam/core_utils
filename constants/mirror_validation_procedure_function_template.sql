...
def compare_csv_with_table(session: Session,dataset_name, stage_name,database,schema, table_name,run_date,header_row_no=1):
    ...
    for full_file_path in list_of_files:
        ...
        tr_table_query = f"""select {table_cols} from {database}.{schema}.{table_name} where FILE_DATE = '{run_date}' """
